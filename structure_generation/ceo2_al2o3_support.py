...
def main():
    ...
    # Generate Al2O3 slab
    al2o3_slab = generator.generate_al2o3_slab("../../../2_NP_model/input/Al2O3.poscar")
    ...
    # Save structures
    write('al2o3_slab.xyz', fixed_slab)
    
    # Example cluster sizes to analyze
    spherical_sizes = [12, 15, 17, 23, 25, 32]
    
    # Read clusters and create cut versions
    cut_spherical_clusters = []
    for size in spherical_sizes:
        cluster = read(f'../../../2_NP_model/input/spherical/CeO2_{size}.xyz')
        ...
        write(f'./output/cluster/ceo2_cut_{size}.xyz', cut_cluster)
    ...
    for i, cluster in enumerate(cut_spherical_clusters):
        ...
        write(f'./output/cluster/ceo2_supported_{i}.xyz', supported)
    ...
    for i, structure in enumerate(supported_list):
        ...
        traj_filename = f"./output/md_str_opt_20ps_{i}.traj"
        log_filename = f"./output/str_opt_20ps_{i}.log"
