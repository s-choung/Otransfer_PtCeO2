...
def main():
    ...
    size_names = ['37', '57', '73']
    for i in size_names:
        traj = Trajectory(f"../../1_initial_supported_structure/c_md_relax_output/ce3p_per40/2_10per_{i}CA_optimized.traj")
        structure = traj[-1]
    ...
    for i, structures in enumerate(Pt_CeO2_supported_5per_rand_list):
        for j, structure in enumerate(structures):
            write_proteindatabank(f'./test/test{i}{j}_test.pdb', structure)
        write(f'./new_pt_num/{name}_new_Pt_rand_list_{i}__test.traj', structures)
    ...
    for i, structures in enumerate(Pt_CeO2_supported_5per_rand_list):
        ...
        write(f'./new_pt_num/{name}_size_{i}.traj', best_structure)
